...
# plot
a<-ggplot(LCW_LF_11_18_alpha_diversity, aes(x=sample_type, y=pielou, fill=sample_type))+
  geom_boxplot()+
  theme_bw()+
  theme(legend.position = "none", text = element_text(family = "sans", size = 12))+
  scale_fill_manual(values = c("orange", "blue"))+
  labs(y = "Pielou Eveness")
b<-ggplot(LCW_LF_11_18_alpha_diversity, aes(x=sample_type, y=Observed, fill=sample_type))+
  geom_boxplot()+
  theme_bw()+ theme(legend.position = "none")+
  scale_fill_manual(values = c("orange", "blue"))+
  labs(y = "Richness")
c<-ggplot(LCW_LF_11_18_alpha_diversity, aes(x=sample_type, y=Shannon, fill=sample_type))+
  geom_boxplot()+
  theme_bw()+ theme(legend.position = "none")+
  scale_fill_manual(values = c("orange", "blue"))+
...
# first, need to merge samples across treatment groups
ART_merged = merge_samples(ART, "treatment")

# repair merged values
# now using treatment_numeric - 1 is MIC
sample_data(ART_merged)$treatment <- c("MIC")

# transform to %
ART_merged = transform_sample_counts(ART_merged, function(x) 100 * x/sum(x))

# select only top 20 most abundant OTUs
top20otus = names(sort(taxa_sums(ART_merged), TRUE)[1:20])
taxtab20 = cbind(tax_table(ART_merged), family20 = NA)
taxtab20[top20otus, "family20"] <- as(tax_table(ART_merged)[top20otus, "Family"], 
                                      "character")
tax_table(ART_merged) <- tax_table(taxtab20)

# plot
title = "Taxa Barplot: Artemia (MIC)"
plot_bar(ART_merged, "treatment", fill = "family20", title = title) + coord_flip()
